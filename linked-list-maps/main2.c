...
It takes in coordinates and radius in the command line and outputs the coordinates within the radius range into the output file
It prints to stdout the amount of comparisons it does for each search

To run the program type (in terminal):
   1. make map2
   2. ./map2 dataset.csv outputfile < keyfile
                       or
     ./map2 dataset.csv outputfile, then type in the keys (ctrl+d to end)
*/
...
int main(int argc, char * argv[]) {
    ...
    get_input_and_search_radius(tree, output_file);
